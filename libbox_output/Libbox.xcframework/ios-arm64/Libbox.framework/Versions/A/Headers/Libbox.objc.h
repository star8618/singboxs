 ...
 * 🔥 新增：节点切换回调
 */
- (void)onNodeSwitched:(NSString* _Nullable)fromNode toNode:(NSString* _Nullable)toNode;
 ...
 */
@interface LibboxDebugServer : NSObject <goSeqRefInterface> {
}
@property(strong, readonly) _Nonnull id _ref;
 ...
 * DeleteConfig 删除配置文件
 */
- (BOOL)deleteConfig:(NSString* _Nullable)name error:(NSError* _Nullable* _Nullable)error;
/**
 * DownloadConfig 下载配置文件
 */
- (NSData* _Nullable)downloadConfig:(NSString* _Nullable)name error:(NSError* _Nullable* _Nullable)error;
/**
 * GetPort 获取当前端口
 */
- (long)getPort;
 ...
 */
- (NSString* _Nonnull)getStatusJSON;
/**
 * IsRunning 检查是否运行中
 */
- (BOOL)isRunning;
 ...
 * Restart 重启服务器
 */
- (BOOL)restart:(long)newPort error:(NSError* _Nullable* _Nullable)error;
/**
 * SetBasePath 设置 App Group 容器路径
 */
- (void)setBasePath:(NSString* _Nullable)path;
/**
 * Start 启动调试服务器
 */
- (BOOL)start:(long)port error:(NSError* _Nullable* _Nullable)error;
/**
 * Stop 停止调试服务器
 */
 ...
 * UploadConfig 上传配置文件
 */
- (BOOL)uploadConfig:(NSString* _Nullable)name content:(NSData* _Nullable)content error:(NSError* _Nullable* _Nullable)error;
@end
 ...
 * 直连流量统计
 */
@property (nonatomic) int64_t directUplinkTotal;
@property (nonatomic) int64_t directDownlinkTotal;
/**
 * 代理流量统计
 */
@property (nonatomic) int64_t proxyUplinkTotal;
@property (nonatomic) int64_t proxyDownlinkTotal;
/**
 * DNS统计
 */
@property (nonatomic) int64_t dnsTotalQueries;
@property (nonatomic) int64_t dnsSuccessQueries;
@property (nonatomic) int64_t dnsCachedQueries;
/**
 * 节点状态 (0=未知, 1=正常, 2=失败)
 */
@property (nonatomic) int32_t outboundStatus;
/**
 * 节点延迟 (毫秒)
 */
@property (nonatomic) int32_t outboundDelay;
@end
 ...
 */
FOUNDATION_EXPORT long LibboxDebugServerGetPort(void);

/**
 * DebugServerGetStatusJSON 获取状态 JSON（iOS 调用）
 */
FOUNDATION_EXPORT NSString* _Nonnull LibboxDebugServerGetStatusJSON(void);

/**
 * DebugServerIsRunning 检查是否运行中（iOS 调用）
 */
FOUNDATION_EXPORT BOOL LibboxDebugServerIsRunning(void);

/**
 * DebugServerRestart 重启调试服务器（iOS 调用）
 */
FOUNDATION_EXPORT BOOL LibboxDebugServerRestart(long port, NSError* _Nullable* _Nullable error);

/**
 * DebugServerSetBasePath 设置基础路径（iOS 调用）
 */
FOUNDATION_EXPORT void LibboxDebugServerSetBasePath(NSString* _Nullable path);

/**
 * DebugServerStart 启动调试服务器（iOS 调用）
 */
FOUNDATION_EXPORT BOOL LibboxDebugServerStart(long port, NSError* _Nullable* _Nullable error);

/**
 * DebugServerStop 停止调试服务器（iOS 调用）
 */
FOUNDATION_EXPORT BOOL LibboxDebugServerStop(NSError* _Nullable* _Nullable error);
