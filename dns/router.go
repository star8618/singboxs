...
func NewRouter(ctx context.Context, logFactory log.Factory, options option.DNSOptions) *Router {
	...
	// 设置缓存命中回调
	client.cacheHitCallback = func() {
		router.cachedQueries.Add(1)
		router.successQueries.Add(1)
	}
...
func (r *Router) Exchange(ctx context.Context, message *mDNS.Msg, options adapter.DNSQueryOptions) (*mDNS.Msg, error) {
	...
	// 先尝试快速缓存查询（仅用于非独立缓存模式）
	response, cached := r.client.ExchangeCache(ctx, message)
	if cached {
		// 统计缓存命中
		r.cachedQueries.Add(1)
		r.successQueries.Add(1)
		r.logger.DebugContext(ctx, "DNS缓存命中(ExchangeCache): ", FormatQuestion(message.Question[0].String()))
	}
	if !cached {
	...
	if !cached && response != nil {
		r.successQueries.Add(1)
		r.logger.DebugContext(ctx, "DNS查询成功: ", FormatQuestion(message.Question[0].String()))
	}
...
func (r *Router) GetDNSStats() (total, success, cached int64) {
	total = r.totalQueries.Load()
	success = r.successQueries.Load()
	cached = r.cachedQueries.Load()
	// 调试日志
	if total > 0 {
		r.logger.Debug("DNS统计: total=", total, ", success=", success, ", cached=", cached)
	}
